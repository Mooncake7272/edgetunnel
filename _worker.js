		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				const 验证登录状态 = async () => {
					...
					const [hash, expire] = authCookie.split('.');
					if (!expire || Date.now() > Number(expire)) return false;
					return hash === await MD5MD5(UA + 加密秘钥 + 管理员密码 + expire);
				};
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (await 验证登录状态()) return new Response('重定向中...', { status: 302, headers: { 'Location': '/admin' } });
					if (request.method === 'POST') {
						...
						if (输入密码 === (typeof 管理员密码 === 'string' ? 管理员密码.replace(/[\r\n]/g, '') : 管理员密码)) {
							...
							const expireTime = Date.now() + 86400000; // 24小时后过期
							响应.headers.set('Set-Cookie', `auth=${await MD5MD5(UA + 加密秘钥 + 管理员密码 + expireTime)}.${expireTime}; Path=/; Max-Age=86400; HttpOnly; Secure; SameSite=Strict`);
							return 响应;
						}
					}
					...
					const 作为优选订阅生成器 = ['1', 'true'].includes(env.BEST_SUB) && url.searchParams.get('host') === 'example.com' && url.searchParams.get('uuid') === '00000000-0000-4000-8000-000000000000' && 请求TOKEN === 订阅TOKEN;
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							if (!url.searchParams.has('sub') || true) { // 强制本地生成订阅
					...
					if (await 验证登录状态()) return fetch(new Request('https://speed.cloudflare.com/locations', { headers: { 'Referer': 'https://speed.cloudflare.com/' } }));
